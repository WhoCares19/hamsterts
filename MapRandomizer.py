...
GAME_TIMER_DURATION = 150.0 # Seconds before enemies attack
...
timer_font = pygame.font.Font(None, 32) # Slightly larger for timer text
...
def generate_random_features(grid, rows, columns, exclusion_coords):
    features = []
    occupied_coords = set(exclusion_coords) 
    
    for r in range(rows):
        for c in range(columns):
            if grid[r][c] == "grass" and (r, c) not in occupied_coords: 
                roll = random.random()
                if roll < 0.01: 
...
def main():
    ...
    def spawn_entities():
        ...
        # ENEMIES REMOVED FROM INITIAL SPAWN
    ...
    def spawn_enemy_wave(count=10):
        ...
        # Fallback if column 0 is blocked (shouldn't be typically, but just in case)
